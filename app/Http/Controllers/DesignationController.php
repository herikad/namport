<?php

namespace App\Http\Controllers;
use App\Http\Controllers\Controller;
use App\Models\Designation;
use App\Models\MstClient;
use Illuminate\Http\Request;
use Log;
use Illuminate\Support\Facades\Validator;

class DesignationController extends Controller
{
    public function index(Request $request){
      try {
          return view('pages.designation.index');
      } catch (\Exception $e) {
          Log::info("designation_index_error ". print_r($e->getMessage(), true));
      }
    }

    public function create($id=''){
        try {
           
            $clients = MstClient::where('is_active', 1)->get(['client_id', 'display_name','company_name']);
            if(!empty($id)){
                $designation = Designation::where('designation_id', $id)->first();
                return view('pages.designation.create',compact('clients','department'));
            } else {
            return view('pages.designation.create',compact('clients'));
            }

        } catch (\Throwable $th) {
            Log::info("designation_update_create_error ". print_r($th->getMessage(), true));
            return redirect()->route('setup.designation.index')->with("message" , trans('pages.something_wrong'));
        }
    }

    public function designation_json_list(Request $request){
        try {
            $page_index = (int)$request->input('start') > 0 ? ($request->input('start') / $request->input('length')) + 1 : 1;

            $limit = (int)$request->input('length') > 0 ? $request->input('length') : DEFAULT_RECORDS_LIMIT;
            $columnIndex = $request->input('order')[0]['column']; // Column index
            $columnName = $request->input('columns')[$columnIndex]['data']; // Column name
            $columnSortOrder = $request->input('order')[0]['dir']; // asc or desc value

            $main_query =  Designation::from('mst_designation as designation')
                                    ->join('mst_client as client', 'designation.client_id', '=','client.client_id')
                                    ->select(
                                        'designation.designation_id',
                                        'designation.designation_name',
                                        'designation.is_active',
                                        'designation.created_at',
                                        'designation.client_id',
                                        'client.company_name'
                                    );


            $main_query =  $main_query->orderBy($columnName, $columnSortOrder);

            $data_list_for_count = $main_query->get();  // group by and direct count not working
            $recordsTotal = count($data_list_for_count);

            $recordsFiltered = $recordsTotal;

            if(empty($request->input('search.value'))){

                $appointments = $main_query->paginate($limit, ['*'], 'page', $page_index);

            }else {

                $search = $request->input('search.value');

                $search_query = $main_query->where(function ($query) use ($search) {
                    $query->where('designation.designation_name', 'LIKE', "%{$search}%")
                        ->orWhere('client.company_name', 'LIKE', "%{$search}%");
                });

                $appointments = $search_query->paginate($limit, ['*'], 'page', $page_index);

                $search_list_for_count = $search_query->get();  // group by and direct count not working

                $recordsFiltered = count($search_list_for_count);

            }

            $response = array(
                "draw" => (int)$request->input('draw'),
                "recordsTotal" => (int)$recordsTotal,
                "recordsFiltered" => (int)$recordsFiltered,
                "data" => $appointments->getCollection()
            );

            return response()->json($response, 200);
        } catch (\Exception $e) {
            Log::info("designation_json_list_error ". print_r($e->getMessage(), true));
        }
    }

    public function status_update(Request $request){
        try {
            $status_update = Designation::where('designation_id', $request->designation_id)->first();

            if ($status_update) {

                $status_update->is_active = $request->is_active;
                $status_update->save();

                return response()->json(['status' => 1, 'message' => trans('pages.crud_messages.is_active_update', ['attr' => 'Designation']), 'data' => $request->is_active], 200);

            } else {

                return response()->json(['status' => 0, "message" => trans('pages.crud_messages.no_data', ['attr' => 'Department'])], 200);
            }
        } catch (\Exception $e) {

            Log::info("designation_status_update error " . print_r($e->getMessage(), true));

            return redirect()->back()->with(["success" => 0, "message" => trans('pages.something_wrong')]);

        }
    }
}
