...
@if($page == 1 && count($list) == 0)
    ...
    <p class="post_not_found text-center">Post Not Found!</p>
@else
    ...
    <div class="row border mb-2">
        @if($post->type_of_post == config('custom.type_of_post.normal') )
            @if (isset($post->media_files) && count($post->media_files) > 0)
                <div id="post_image_block_{{ $key }}" class="carousel slide p-2" data-bs-ride="carousel">
                    ...
                    @if (count($post->media_files) > 1)
                        <button class="carousel-control-prev" type="button"
                            data-bs-target="#post_image_block_{{ $key }}" data-bs-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next" type="button"
                            data-bs-target="#post_image_block_{{ $key }}" data-bs-slide="next">
                    ...
                    <div class="card-body d-flex  p-1 justify-content-center">
                        @if(isset($post->score_details) &&  isset($post->score_details['score']))
                            ...
                            @foreach ($post->score_details['score'] as $k2 => $v2 )
                                ...
                                <div class="card col-6 m-1 shadow-none border">
                                    ...
                                    <div class="card-footer p-0">
                                        ...
                                        @if(isset($v2['score']))
                                            @foreach ( $v2['score'] as $k4 => $v4 )
                                                    <div class="col-md-4 text-center border p-2"><h5 class="mb-0">{{ $v4 }}</h5></div>
                                            @endforeach
                                        @endif
                                    </div>
                                    </div>
        ...
        <div class="comment text-start p-2 d-flex justify-content-between">
            <div class="clickable show_liked_users"  data-id="{{$post->post_id}}">
                <i class="bx bx-heart fs-4"></i> {{ isset($post->total_likes) ? $post->total_likes : '0' }} Likes
            </div>
            <div class="clickable show_commented_users" data-id="{{$post->post_id}}">
            ...
            @if($post->type_of_post == config('custom.type_of_post.interest_activity'))
                <div class="clickable show_interested_users" data-id="{{$post->post_id}}">
